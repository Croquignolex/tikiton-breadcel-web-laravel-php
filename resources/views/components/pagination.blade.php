    ...
    <div class="pagination">
        <ul>
            @if($paginationTools->previousPage !== 0)
                <li>
                    <a href="{{ $paginationService->getPageUrl($paginationTools->url, $paginationTools->previousPage) }}" aria-label="Previous">
                ...
                @for($i = $paginationTools->currentPage -  $paginationTools->itemsBeforeAndAfter; $i < $paginationTools->currentPage; $i++)
                    @if($i > 0)
                        <li>
                            <a href="{{ $paginationService->getPageUrl($paginationTools->url, $i) }}">
                                {{ $i }}
                            </a>
                        </li>
                    @endif
            ...
            @if($paginationTools->nextPage !== 0)
                <li>
                    <a href="{{ $paginationService->getPageUrl($paginationTools->url, $paginationTools->nextPage) }}" aria-label="Previous">
